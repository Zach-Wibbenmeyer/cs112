...
void AccountManager::changeUserName(const string& oldUserName, const string& newUserName) {
	map<string, string>::iterator it = myAccounts.find(oldUserName);
	if (it != myAccounts.end() ) {
		string password = it->second;
		deleteAccount(oldUserName);
		createNewAccount(newUserName, password);
	} else {
		throw Exception("changeUserName", string("user ") + oldUserName + " is not in the system");
	}
}
...
void AccountManager::deleteAccount(const string& userName) {
	map<string, string>::iterator it = myAccounts.find(userName);
	if (it != myAccounts.end() ) {
		myAccounts.erase(it);
	} else {
		throw Exception("deleteAccount", string("user ") + userName + " is not in the system");
	}
}
...
void AccountManager::changePassword(const string& userName,
	...
	map<string, string>::iterator it = myAccounts.find(userName);
	if (it != myAccounts.end() ) {
		myAccounts[userName] = newPassword;
	} else {
		throw Exception("changePassword", string("user ") + userName + " is not in the system");
	}
}
